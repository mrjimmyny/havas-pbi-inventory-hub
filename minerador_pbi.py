...
def clean_name(raw_name):
    ...
    if "lineageTag" in raw_name: raw_name = raw_name.split("lineageTag")[0]
...
def get_human_visual_type(raw_type):
    ...
    if not raw_type: return "Visual Desconhecido"
...
def scan_report_hierarchy_v31(root_path, all_measures_names):
    ...
    # 2. Varredura
    for dirpath, _, filenames in os.walk(root_path):
        if "SemanticModel" in dirpath: continue
        ...
        for filename in filenames:
            ...
            if not is_visual_file: continue
            ...
            # B. Lê Conteúdo
            try:
                with open(filepath, "r", encoding="utf-8-sig") as f: file_content = f.read()
            except: continue
            ...
            for m in all_measures_names:
                m_lower = m.lower()
                if f'"{m_lower}"' in lower_content or f"'{m_lower}'" in lower_content or f"[{m_lower}]" in lower_content:
            ...
            if measures_found:
                ...
                if type_match:
                    ...
                    # Se falhar o regex, procura nomes conhecidos no texto bruto
...
def parse_tmdl_structure(root_path):
    ...
    re_rel_block = re.compile(r"relationship\s+.*?(?=\n\s*(?:relationship|table|measure|\Z))", re.DOTALL | re.IGNORECASE)

    for dirpath, _, filenames in os.walk(root_path):
        for filename in filenames:
            if not filename.endswith(TMDL_EXT): continue
            filepath = os.path.join(dirpath, filename)
            try:
            ...
            if "tables" in dirpath:
                ...
                if current_table not in tables_data: tables_data[current_table] = {"columns": []}
                ...
                for line in lines:
                    ...
                    if strip.startswith("column "):
                        if curr_col: tables_data[current_table]["columns"].append(curr_col)
                        raw_c = strip.replace("column ", "")
                        is_calc = "=" in raw_c
                        c_name = clean_name(raw_c.split('=')[0])
                        curr_col = {"name": c_name, "type": "string", "origin": "Calculada (DAX)" if is_calc else "Física"}
            ...
            for r in re_rel_block.finditer(content):
                ...
                if fc and tc:
                    relationships.append({
                        "from": clean_ref(fc.group(1)), "to": clean_ref(tc.group(1)),
    ...
    print(f"> Tabelas: {len(tables_data)} | Rels: {len(unique_rels)} | Medidas: {len(measures)}")
    return {"tables": tables_data, "relationships": unique_rels, "measures": measures}
...
def analyze_and_map(inventory, root_path):
    ...
    for i, m in enumerate(inventory["measures"]):
        ...
        for other in all_names:
            ...
            if re.search(r'\[\s*' + re.escape(other) + r'\s*\]|"\s*' + re.escape(other) + r'\s*"', m["dax"], re.IGNORECASE):
    ...
    for m in enhanced:
        ...
        if (not m["parent_names"]) and (not children) and (not m["in_visual"]):
            ...
            candidates += 1
        elif m["in_visual"]: m["status"] = "Visual"
        elif children: m["status"] = "Base Cálculo"
        else: m["status"] = "Dependente"
...
def save_outputs(inv):
    with open("model_structure.json", "w", encoding="utf-8") as f: json.dump(inv, f, indent=4)
